...
- 路径一律相对本技能根目录：本技能脚本写 `scripts/x.py`，web-fetch 基座写 `../web-fetch/scripts/x.py`（两者恒为兄弟目录）。执行时给命令加上技能根目录前缀，不要 `cd` 进技能目录——输出文件要落在当前工作目录。前缀取宿主加载技能时告知的 Base directory；拿不到就用 `find / -name departments.py -not -path '*__pycache__*' 2>/dev/null | head -1` 定位，取其上两级为技能根目录。
...
- 附件链接缺失：正文页结果无 `attachments`、正文里也找不到附件链接时，如实说明「未获取到附件原文」，只就通知正文可见的内容解读
