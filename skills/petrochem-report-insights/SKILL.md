...
description: 从中国石化、中国石油、中国海油、中化集团、延长石油等国内石化企业，埃克森美孚、壳牌、BP、道达尔、雪佛龙等国际油气巨头，以及中国石油和化学工业联合会、IEA、OPEC、伍德麦肯兹等行业研究机构的公开报告（年报、可持续发展报告、新闻稿、月度报告、研究报告）中检索并摘取关键洞察，从核心数据、关键结论、市场趋势、投资动态、风险、产业链影响六个维度分析，输出深度分析报告 / 行业动态速览 / 多机构观点对比三类报告。当用户提到石化报告、能源研报、油气行业研究、石化市场洞察、炼化行业分析、油气企业年报解读、行业动态摘要时触发。
metadata:
  title: 石化行业研报洞察助手
  opening: |
...
发现候选统一走 `web-fetch` 的 360 搜索（用机构的 `site_domain` 限定），不针对单个机构维护官网抓取正则——国内五家企业官网在当前网络环境下普遍无法脚本直抓（TLS 握手失败/反爬/JS 空壳），完全依赖 360 搜索发现（它也会命中企业官网之外的第三方披露/转载源）；国际油气巨头官网多可直接被 `web-fetch` 的 urllib/curl_cffi 层抓到。
...
`--site` 传对应机构的 `site_domain`。`errors[].kind = no_match` 换关键词重试；`blocked`/`network_unreachable` 说明该机构当前部署环境下抓不到（先跑 `uv run ../web-fetch/scripts/fetch.py --check-env` 确认是否缺 `curl_cffi`/`playwright`），如实告知用户，不要编造内容替代。
...
- 检索/抓取受阻：区分是"该机构无匹配结果"还是"当前环境三层引擎都过不了该站点"，后者需如实告知环境限制
