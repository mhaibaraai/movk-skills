...
from engines import fetch_one, probe_engines


def parallel_fetch(urls: list[str], max_chars: int, max_pages: int, engine: str, max_workers: int) -> list[dict]:
    deduped = list(dict.fromkeys(urls))
    print(f"抓取 {len(deduped)} 个页面（engine={engine}, max_workers={max_workers}）...", file=sys.stderr)

    results: list[dict] = [{}] * len(deduped)
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(fetch_one, u, max_chars, max_pages, engine): i
...
def main() -> None:
    ...
    payload = parallel_fetch(urls, args.max_chars, args.max_pages, args.engine, args.max_workers)
