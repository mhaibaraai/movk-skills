...
# 渲染出正文后可以洗白的状态码：瑞数首跳 412、Cloudflare 403/503，挑战通过后首跳状态码
# 不代表最终结果。404/410 不在其列——那是「资源不存在」的确定信号，站点错误页跳转首页后
# 照样渲染得出正文，洗白它等于把一个无关页面当成功返回（gov.cn 的 404 页即如此）。
CHALLENGE_STATUS = frozenset({403, 412, 429, 503})
...
def _is_pdf(data: bytes, content_type: str) -> bool:
    """认魔数而不只认 Content-Type——政府站常把 PDF 标成 application/octet-stream，
    只信 Content-Type 会让 PDF 二进制走进 HTML 分支，被解码成一篇乱码「正文」。"""
...
async def _browser_fetch_one(ctx, url: str, timeout: int) -> tuple[bytes, str, int]:
    ...
    try:
        ...
        return data, content_type or "text/html; charset=utf-8", _resolve_browser_status(status, passed)
    finally:
        await page.close()


def _resolve_browser_status(status: int, passed: bool) -> int:
    """渲染出正文后，首跳的状态码还算不算数。

    算不算，取决于它是哪一种 4xx。挑战类（瑞数 412、CF 403/503）首跳返回的是混淆脚本，
    脚本跑完重载才有真内容，此时首跳状态码确实不代表最终结果，洗白成 200 是对的。
    但 404 是「资源不存在」——站点的错误页照样能渲染出满屏正文（gov.cn 的 404 页直接
    JS 跳首页），洗白它就是把中国政府网首页当作政策原文交给下游解读。

    正文没渲染出来时一律保留原状态码，绝不把空壳洗白成 200。
    """
    if not passed:
        return status
    if status >= 400 and status not in CHALLENGE_STATUS:
        return status
    return 200
...
def _blocked_reason(status: int, data: bytes, content_type: str,
                    expect: re.Pattern | None = None, url: str = "") -> tuple[str, str] | None:
    ...
    url 让「要的是什么」也成为一道哨兵：请求 .pdf 却拿回 HTML，多半是链接失效或被重定向到
    错误页，那页 HTML 再完整也不是调用方要的东西。
    """
    ...
    if url.split("?")[0].lower().endswith(".pdf"):
        return "wrong_content_type", "请求 PDF 但服务端返回非 PDF 内容，多为链接失效或被重定向到错误页"
...
def fetch_many_bytes(urls: list[str], engine: str = "auto", timeout: int = 20,
    ...
    for eng in chain:
        ...
        for url in pending:
            ...
            reason = _blocked_reason(status, data, content_type, expect, url)
            if reason is None:
...
def _raw_one(fetched: dict) -> dict:
    ...
    url = fetched["url"]
    content_type = fetched.get("content_type") or ""
    if _is_pdf(fetched["data"], content_type):
