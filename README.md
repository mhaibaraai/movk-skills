...
| [create-ppt](skills/create-ppt/SKILL.md) | 企业 PPT 助手。按「选模板 → 确认意图 → 生成大纲 → 确认执行 → 渲染」产出 PPT，内置 5 套真实设计模板（安全培训、项目立项、年终汇报、专项汇报、通用）。复用模板已设计好的页面（克隆 + 文本回填），产物与模板视觉一致且可二次编辑；支持上传 word/大纲或粘贴文稿直接成稿，用户自带模板经 `index_template.py` 索引后即可接入。 |
