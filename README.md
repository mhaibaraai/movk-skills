...
| [file-extract](skills/file-extract/SKILL.md) | 通用文件解包与抽取基座。给 URL 或本地路径，返回压缩包内全部材料的结构化文本：zip 递归解压（修复中文文件名乱码）、PDF / docx 抽文本，图片与扫描件本机联网时走 rapidocr、离线时交回平台识别。脚本零第三方依赖，离线环境直接可跑。 |
| [contract-review](skills/contract-review/SKILL.md) | 合同辅助审查。解析「合同 + 签约依据」压缩包，逐条对照公司标准合同模板，输出覆盖基础文本问题、条款缺失、依据一致性、风险提示的 Markdown 审查报告。依赖 file-extract 基座，四类标准合同模板随技能包分发。 |
