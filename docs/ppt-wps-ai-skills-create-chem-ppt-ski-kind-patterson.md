...
AI对话[主对话A / 主对话B]  两份配置完全一致（仅所处分支不同）
   Skill: create-ppt + web-fetch；历史记录=工作流（多轮）；返回内容=开
   ...
   系统角色管两道门 + 结束意图判定，逐轮推进：
     ...
     · 主题要求含真实数据且 g_source 为空 → 意图卡加第 6 项「数据来源」，选定联网后用 web-fetch 取公开数据，带来源标注
...
- **web-fetch 只挂主对话，不挂渲染节点**：联网取数发生在门②填充之前，取回的数据要经用户在卡片上确认。挂到渲染节点等于授权它在拿不到大纲时自己上网编一份——循环是没了，但客户确认的大纲与最终成品不再是同一份，问题更隐蔽。渲染节点的职责边界就是「重建 + 渲染 + 发布」，不碰内容。
...
**AI 对话[主对话A / 主对话B]** — 全流程核心，**两份配置完全一致**（分支不合流才复制，改一处须同步另一处）。模型 `energy-deepseek-v3`（能编排 skill 又能写作）；**Skill 挂 create-ppt + web-fetch**（create-ppt 做 make_outline，web-fetch 供门①确认后的公开数据检索；不挂 download-link-publisher）；**历史聊天记录＝工作流、10 轮**；返回内容＝开；开启思考＝关。
...
- 主题要求含真实数据而上传素材为空时，意图卡追加第 6 项「6. 数据来源：公开行业数据（联网检索，将标注出处）」，可选值：公开行业数据（联网检索）／您上传的内部数据／暂缺（留占位）。用户选定公开行业数据后，在填充 outline.json 之前用 web-fetch 技能检索（sitemap → search → fetch），只取权威公开源，每条数据带短来源标注且计入 caps；检索报 blocked/no_match 就如实告知「联网检索未取到数据」并请其上传或确认留占位，绝不编造、绝不用行业数据冒充公司内部数据。详见 skills/create-ppt/references/writing-guide.md 的「联网取数与来源标注」。
